...
if nargin == 2 || (nargin == 3 && isempty(dzdy))
    Temp = X-c; 
    Y = 1 / 2 *sigma2* sum(Temp(Temp < 1/sigma2) .^ 2)+sum(abs(Temp(Temp >= 1/sigma2))-0.5/sigma2);
...
elseif nargin == 3 && ~isempty(dzdy)
    ...
    Y = dzdy *sigma2* (X - c);
    Temp = X-c;
    Y(abs(Temp) >= 1/sigma2) = sign(Temp(abs(Temp) >= 1/sigma2));
    % Y is divided by d(4) in cnn_train.m / cnn_train_mgpu.m.
